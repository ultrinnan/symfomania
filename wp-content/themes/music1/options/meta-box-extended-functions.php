...
function get_the_events($month,$year) {
	...
	$startepoch = strtotime($dateholder[0] .  ' ' .  date( 'F', mktime(0, 0, 0, $dateholder[1], 1) ) . ' ' .  $dateholder[2] .  ' ' . $key_time_value . ' GMT');
		
	$endepoch = '';	
	if ($key_end_value) {
		$endepoch = strtotime($endholder[0] .  ' ' .  date( 'F', mktime(0, 0, 0, $endholder[1], 1) ) . ' ' .  $endholder[2] .  ' ' . $key_time_value . ' GMT');
	} 
...
function get_the_calendar($cmonth,$cyear) {
	...
	if($calentries) {		
		...
		foreach ($calentries as $cal_the_entry) {
			...
			<span class="cweekname smallfont">' . date_i18n( 'D' , $cal_the_entry['strdate'] , false ) . '<br/>' . $caltime . '</span>
			...
			if ($calmap) {
				...
				<input type="submit" class="smallfont" name="nets_mapsubmit" value ="Get directions" id="nets_mapsubmit" title="' . __('Click for directions ', 'localize') . '"">
				</form>';
			}
			$output .= 	'
...
function get_for_timer($title){
		...
		if($calentries) {	
			...
			foreach ($calentries as $cal_the_entry) {			
				if ($cal_the_entry['strdate'] >= $currententry && !$ccounter) {	
					...
					$output .= '
					<div class="timernames smallfont">';
					$output .= '
					<span class="daynames first">' .  __('DAYS', 'localize') . '</span>
					<span class="daynames second">' .  __('HOURS', 'localize') . '</span>
					<span class="daynames third">' .  __('MINS', 'localize') . '</span>
					<span class="daynames fourth">' .  __('SECS', 'localize') . '</span>
					</div></div><div class="arrow-left smallfont"><a href="' .  get_permalink($settings['ntl_calendar_page'])  . '">' .   $settings['ntl_calmoreinf_label']    . '</a></div></div>';
					$ccounter = 1;			
				}		
			}	
		}		
	...
	if ($output){
		...
		$output = '<div class="timeshow">' . $output . '</div><div class="clear"></div>';
...
function get_for_widget($num, $offset){
	...
	foreach ($calwidget as $calwidg) {
		...
		if ($calcounter >= $offset) {
			...
			<span class="cweekname smallfont">' . date_i18n( 'M' , $calwidg['wstrdate'] , false ) . '<br/>' . $caltime . '</span>	
		...
		if ($s7 == 'content'){
			...
			switch($s8) {
				...
				case 't1': 
					...
					if ($e_meta) {
						$content .= '<a href="'  .  get_permalink( $e_meta )  . '" class="smallfont">more</a></div></li>';
					}							
				break;
					...
					if ($a_meta || $b_meta || $c_meta){
						$content .= '
							<span>
								...
								<a href="'  .  get_permalink( $e_meta )  . '" class="smallfont">more</a>
							</span>
						';
					}
